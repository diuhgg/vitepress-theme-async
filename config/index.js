...
var EXTERNAL_URL_RE, withBase, joinPath, isString, isObject, colors;
...
var less_default = (config) => {
  if (config.vite?.css?.preprocessorOptions?.less.globalVars) {
    const processThemeColor = (color, defaultValue) => {
    ...
    const setThemeGlobalVars = (colorConfig) => {
      Object.entries(colorConfig).forEach(([key, value]) => {
        if (key === "enable")
          return;
        const [light, dark] = processThemeColor(value, defaultColor[key]);
